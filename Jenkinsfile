                ...
                script{
                    try {
                       // withAWS(credential: '${AWS_CREDENTIALS}', role: 'arn:aws:iam::347222812711:user/cosmost-depoly', roleAccount: '347222812711', externalId:'externalId') {
                            sh 'aws ecr get-login-password --region ap-northeast-2 | docker login --username AWS --password-stdin ${ENCRYPTED_TOKEN} 347222812711.dkr.ecr.ap-northeast-2.amazonaws.com'
